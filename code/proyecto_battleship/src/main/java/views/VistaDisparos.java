 ...
 */
public class VistaDisparos extends javax.swing.JPanel implements IObserver {
    
    private static final java.util.logging.Logger logger = java.util.logging.Logger.getLogger(VistaDisparos.class.getName());
     ...
     * @param nombreOponente Nombre del jugador oponente.
     * @param controlador Controlador del patrón MVC
     * @param naves Naves a cargar en el tablero de naves.
     */
    public VistaDisparos(String nombreJugador, String nombreOponente, ControlDisparo controlador, NaveDTO[] naves) {
        ...
        // Se suscribe como observador de la partida.
        controlador.getPartida().agregarObserver(this);
        
        // Se obtiene el turno actual de la partida.
        actualizarTurnoDesdeDTO(controlador.obtenerTurnoActual());
        
        // Cargar los recursos y elementos del panel
        inicializarComponentes(naves);
    ...
    /**
     * Reincia el panel para una nueva partida.
     * @param nombreJugador Nombre del jugador actual.
     * @param nombreOponente Nombre del jugador oponente.
     * @param naves Naves a cargar en el tablero de naves.
     */
    ...
    private void inicializarComponentes(NaveDTO[] naves) {
        add(crearPanelSuperior(), BorderLayout.NORTH);
        add(crearPanelCentral(naves), BorderLayout.CENTER);
    ...
    private JPanel crearPanelCentral(NaveDTO[] naves) {
        JPanel panel = new JPanel(new GridLayout(1, 2, 30, 0));
        panel.setBackground(COLOR_FONDO);
        panel.setBorder(BorderFactory.createEmptyBorder(15, 20, 15, 20));

        panel.add(crearTableroDisparos());
        panel.add(crearTableroNaves(naves));

        return panel;
    }

    private JPanel crearTableroDisparos() {
        VistaTableroDisparos tableroDisparos = new VistaTableroDisparos(this);
        panelTableroDisparos = tableroDisparos;
        return crearPanelTablero("TABLERO DE DISPAROS", tableroDisparos);
    }
    
    private JPanel crearTableroNaves(NaveDTO[] naves){
        VistaTableroNaves tableroPropio = new VistaTableroNaves();
        panelTableroPropio = tableroPropio;
        mostrarNavesDesdeDTO(naves);
        return crearPanelTablero("MI TABLERO", tableroPropio);
    }
    
    private JPanel crearPanelTablero(String titulo, JPanel tablero) {
     ...
     */
    private void mostrarNavesDesdeDTO(NaveDTO[] naves) {
        if (naves == null)
            return;
    ...
    private void botonAbandonarBatallaActionPerformed(java.awt.event.ActionEvent evt) {                                                      
        ...
        if (confirmacion == JOptionPane.YES_OPTION) {
            ...
            // Detener el temporizador de la vista
            temporizador.stop();
